...
public class FreeCamera extends AbstractClientPlayer {
    ...
    private void doMotion() {
        switch (FreeCam.getFlightMode()) {
            ...
            case CREATIVE -> {
                ...
                if (this.input.shiftKeyDown ^ this.input.jumping) {
                    int direction = this.input.jumping ? 1 : -1;
    ...
    // Mirrors LocalPlayer.serverAiStep(): copies input to movement fields so travel() moves the FreeCamera.
    @Override
    public void serverAiStep() {
        super.serverAiStep();
        if (MC.getCameraEntity() == this) {
            ...
            this.setSprinting((MC.options.keySprint.isDown() && this.input.up) || (this.input.up && this.isSprinting()));
