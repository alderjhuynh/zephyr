...
public final class InventoryRenderer extends Module {
    ...
    private static int anchorX(GuiGraphics graphics, int totalWidth, Placement placement) {
    ...
    private static int anchorY(GuiGraphics graphics, int totalHeight, Placement placement) {
        return switch (placement) {
    ...
    private void drawArmorColumn(GuiGraphics graphics, Font font, Player player,
    ...
    private void drawSlot(GuiGraphics graphics, Font font, Player player,
                          ItemStack stack, int x, int y, boolean selected) {
        if (selected && highlightSelected.get()) {
            graphics.renderOutline(x, y, ICON_SIZE, ICON_SIZE, SELECTED_SLOT_COLOR);
        }
