...
public final class ItemRestock extends Module {
    ...
    private static void restockHand(Minecraft client, Player player, InteractionHand hand, ItemStack template) {
        ...
        client.gameMode.handleInventoryMouseClick(menu.containerId, sourceScreenSlot, 0, ClickType.PICKUP, player);
        client.gameMode.handleInventoryMouseClick(menu.containerId, targetScreenSlot, 0, ClickType.PICKUP, player);
        client.gameMode.handleInventoryMouseClick(menu.containerId, sourceScreenSlot, 0, ClickType.PICKUP, player);
    }
    ...
    private static int findRestockSource(Inventory inventory, ItemStack template, int excludedInventorySlot) {
        int bestSlot = -1;
        int bestCount = -1;
        int oppositeHandSlot = excludedInventorySlot == Inventory.SLOT_OFFHAND ? inventory.selected : Inventory.SLOT_OFFHAND;
    ...
    private static int getHandInventorySlot(Inventory inventory, InteractionHand hand) {
        return hand == InteractionHand.MAIN_HAND ? inventory.selected : Inventory.SLOT_OFFHAND;
    }
