...
public final class ClickGuiScreen extends ZephyrScreen {
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        withPanelSlide(() -> {
            ...
            if (layout.isEmpty()) {
                centeredText(graphics, this.font, "No modules found", panelX + panelWidth / 2,
                        listTop + 20, TEXT_DIM);
            }
    ...
    private void renderListSetting(GuiGraphics graphics, ListSetting setting, int top, int left, int right) {
        ...
        for (ListSetting.ListEntry entry : setting.get()) {
            ...
            centeredText(graphics, this.font, "x", removeX + LIST_ICON_SIZE / 2, removeY, TEXT_DIM);
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        boolean confirm = keyCode == GLFW.GLFW_KEY_ENTER || keyCode == GLFW.GLFW_KEY_KP_ENTER;
        if (confirm && listEditing != null) {
            commitNewEntry();
            return true;
        }
