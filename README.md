...
A client-side hacked client / utility mod for **Minecraft 1.21.11**, built on [Fabric](https://fabricmc.net/). Zephyr packs 75 modules into four categories, Movement, Combat, QoL, and Disable, with a fully clickable GUI, a chat command system, configurable keybinds, configurable profiles, and Discord Rich Presence.
...
| [Fabric API](https://fabricmc.net/use/)              | any recent build for 1.21.11 |
| Java                                                 | 25+                         |
...
1. Install **Fabric Loader** for Minecraft 1.21.11 using the [Fabric installer](https://fabricmc.net/use/installer/).
2. Install **Fabric API** for Minecraft 1.21.11 (from [Modrinth](https://modrinth.com/mod/fabric-api) or [CurseForge](https://curseforge.com/minecraft/mc-mods/fabric-api)).
...
- Press **`Tab`** while the menu is open to cycle between the other screens (global settings, keybinds, profiles, and back). Holding **`Down`** or **`Up`** while pressing Tab cycles through the two hidden easter-egg screens, and holding **`Left`** cycles backward through the normal screens.
...
Enable it from the click-GUI (Category: QoL) or with `.z module Seedcracker on`. Its settings panel controls which structures and decorators are scanned (Buried Treasure, Desert Temple, End City, Jungle Temple, Monument, Swamp Hut, Shipwreck, Pillager Outpost, Igloo, Trial Chambers, End Pillars, End Gateway, Dungeon, Emerald Ore, Desert Well, Warped Fungus, Biome), the render mode for found structure outlines (`OFF` / `ON` / `XRAY`), and debug/anti-xray options.
...
| `.z seedcracker data restore`                         | Restores structures from the previous session (currently only reports that none were found) |
...
| `.z seedcracker gui`                                  | Prints a notice; the config screen is not available in this port                  |
...
> **Note:** this module is a port of [AppleSkin](https://github.com/squeek502/AppleSkin), adapted to Zephyr's module and rendering systems and 1.21.11 MojMaps.
...
While enabled it renders a preview of the hovered container's inventory in the item tooltip. You can preview a **Shulker Box** (in every dye color), **Chest** (including trapped and all copper variants), **Barrel**, **Furnace**, **Blast Furnace**, **Smoker**, **Dropper**, **Dispenser**, **Hopper**, **Brewing Stand**, **Chiseled Book Shelf**, **Decorated Pot**, every wooden **Shelf**, and any item holding an `ItemContainerContents` component.
